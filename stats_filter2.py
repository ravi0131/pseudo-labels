...
def filter_by_aspect_ratio(df, aspect_ratio_col,min_ratio):
    """
...
def apply_rect_filter(df, aspect_ratio_col, area_col, min_ratio, min_area):
    ...
    """
    # filter out all the boxes that have aspect ratio less than min_ratio and area less than min_area
    filtered_df = df[
        (df[aspect_ratio_col] > min_ratio) &
        (df[area_col] > min_area)
    ].copy()
    return filtered_df
